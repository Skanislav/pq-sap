...
import {
  createWalletClient, encodeFunctionData, formatEther, getAddress, http, parseEther,
  type Address, type Hex,
} from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import { ml_kem512, ml_kem768 } from '@noble/post-quantum/ml-kem.js';

import { publicClientFor, SCHEME_ID, type ChainConfig } from '../lib/chain.ts';
...
import {
  ...
  checkClassicalAnnouncement, randomClassicalSeeds, encodeCompactMeta,
  type ClassicalKeys,
} from '../lib/classical.ts';
import { registerViewingKey } from '../lib/registry.ts';
import {
  fetchDeployment, deriveSpendable, signSpendable, decodePublicKeyData,
  buildSpendUserOp, spendableViewTag, requiredPrefund, ENTRYPOINT_ABI, FACTORY_ABI,
  type DevDeployment,
} from '../lib/spend4337.ts';
import {
  ENTRYPOINT_ABI as HYBRID_ENTRYPOINT_ABI, ZKNOX_FACTORY_ABI, ACCOUNT_ABI as HYBRID_ACCOUNT_ABI,
  preQuantumDemoKey, buildSpendUserOp as buildHybridUserOp, packHybridSignature,
  dummyHybridSignature, requiredPrefund as requiredPrefundHybrid,
...
import { loadClassicalSeeds, saveClassicalSeeds, clearClassicalSeeds } from '../lib/storage.ts';
...
export function SpendTab({ cfg, wallet, ethUsd }: {
  cfg: ChainConfig; wallet: Wallet; ethUsd: number | null;
}) {
  return (
    <section>
      <h2>Spend received payments</h2>
      <p className="lede">
        Two spend routes: the <strong>classical hybrid</strong> spends from a plain
        EOA with one ECDSA transaction (ecrecover, ~21k gas, quantum-vulnerable
        ownership); the <strong>PQ route</strong> spends through an ERC-4337 account
        whose ERC-7913 signer verifies the blinded ML-DSA key on-chain (~15M gas).
      </p>
...
function ClassicalSpend({ cfg, wallet, ethUsd }: {
  cfg: ChainConfig; wallet: Wallet; ethUsd: number | null;
}) {
  ...
  const [spendState, setSpendState] = useState<Record<string, {
    dest: string; amount: string;
    result?: string | undefined; error?: string | undefined; pending?: boolean | undefined;
  }>>({});

  useEffect(() => {
    const seeds = loadClassicalSeeds();
    if (seeds) {
      try { setKeys(deriveClassicalKeys(seeds)); } catch { clearClassicalSeeds(); }
    }
  }, []);
  ...
  const registerCompact = async () => {
    ...
    if (!walletClient?.account) { setError('Connect a wallet (or use the anvil dev account).'); return; }
    setBusy('register');
    try {
      const publicClient = publicClientFor(cfg);
      const meta = decodeClassicalMeta(keys.metaAddress);
      const { index } = await registerViewingKey(
        publicClient, walletClient, cfg, dep.registry, meta.kemEk);
      ...
      try {
        ...
        if (source === 'vector-a') {
          ...
          k = deriveClassicalKeys({
            spendSeed: fromHex(s.spend_seed), kemD: fromHex(s.kem_d), kemZ: fromHex(s.kem_z) });
        } else {
  ...
  const receive = async () => {
    ...
    try {
      ...
      const payTx = await walletClient.sendTransaction({
        account: walletClient.account, chain: cfg.chain, to: addr, value });
      ...
      const annTx = await walletClient.writeContract({
        ...
        args: [SCHEME_ID, addr, toHex(cipherText) as Hex,
          toHex(classicalViewTag(sharedSecret)) as Hex],
      });
  ...
  const scan = async () => {
    ...
    try {
      const publicClient = publicClientFor(cfg);
      const meta = decodeClassicalMeta(keys.metaAddress);
      const { announcements, fromBlock, toBlock } =
        await fetchAnnouncements(cfg, publicClient, (m) => setScanInfo(m));
      ...
      for (const ann of announcements) {
        ...
        found.push({
          ann, sharedSecret: payment.sharedSecret, address,
  ...
  const spend = async (hit: ClassicalHit) => {
    ...
    const patch = (p: Partial<typeof st>) =>
      setSpendState((s) => ({ ...s, [key]: { ...(s[key] ?? st), ...p } }));
    ...
    try {
      ...
      const stealthWallet = createWalletClient({
        chain: cfg.chain, transport: http(cfg.rpcUrl), account: stealthAccount });
      const publicClient = publicClientFor(cfg);
  ...
  return (
    <div>
      ...
      <p className="fine">
        Scheme <code>secp256k1+ML-KEM-768</code> (1,218 B meta-address): discovery is
        post-quantum, the spending key is secp256k1 — <code>p = k + t</code> controls
        the stealth EOA directly.
      </p>
      <div className="row">
        <button className="secondary" disabled={busy !== null} onClick={() => identity('vector-a')}>
          Use test recipient A
        </button>
        <button className="secondary" disabled={busy !== null} onClick={() => identity('random')}>
      ...
      {keys && (
        <>
          ...
          <div className="row">
            <button className="secondary" disabled={busy !== null || !dep?.registry} onClick={registerCompact}>
              {busy === 'register' ? 'Registering…'
                : 'Register viewing key → 65-byte compact meta-address'}
            </button>
          </div>
          {!dep?.registry && (
            <p className="fine">
              (Compact meta-address needs the StealthKeyRegistry — run <code>npm run chain</code>,
              or deploy it to {cfg.label} with <code>npm run deploy:sepolia</code>.)
            </p>
          )}
          {compact && (
            <>
              <HexBlob label={`Compact meta-address (registry #${compact.index})`}
                value={toHex(compact.bytes)} />
              <p className="fine">
                65 bytes: spend pubkey (33 B, SEC1 compressed — its parity byte is the
                version) ‖ registry index (32 B). Share this instead of the 1,218-byte
                form; the viewing key is fetched from the registry by index. A malicious
                registry can only deny detection, never redirect funds — the spend pubkey
                is inline.
              </p>
            </>
          )}
          <div className="row">
            <label className="field narrow" style={{ margin: 0 }}>
              ...
              <input value={receiveAmount} inputMode="decimal"
                onChange={(e) => setReceiveAmount(e.target.value)} />
            </label>
            <button disabled={busy !== null} onClick={receive}>
              {busy === 'receive' ? 'Receiving…' : 'Receive demo payment'}
            </button>
            <button disabled={busy !== null} onClick={scan}>
      ...
      {hits && (
        <Note kind={hits.length ? 'ok' : 'info'}>
          {hits.length
            ? `${hits.length} spendable payment${hits.length === 1 ? '' : 's'} found`
            : 'No payments found'}{scanInfo ? ` — ${scanInfo}` : ''}
        </Note>
      )}
      {hits?.map((h) => {
        ...
        return (
          <div className="hit" key={h.ann.txHash}>
            <div>
              ...
              {' '}<strong>{formatEther(h.balance)} ETH{usd(eth, ethUsd)}</strong>
              <span className="hexmeta"> · block {h.ann.blockNumber.toString()} · plain EOA</span>
            </div>
            <div className="row" style={{ margin: '6px 0 0' }}>
              <input placeholder="destination 0x…" value={st.dest} style={{ flex: 2, minWidth: 220 }}
                onChange={(e) => setSpendState((s) => ({ ...s, [h.ann.txHash]: { ...st, dest: e.target.value } }))} />
              <input placeholder="amount ETH" inputMode="decimal" value={st.amount} style={{ width: 110 }}
                onChange={(e) => setSpendState((s) => ({ ...s, [h.ann.txHash]: { ...st, amount: e.target.value } }))} />
              <button disabled={st.pending || !st.dest.trim() || !st.amount} onClick={() => spend(h)}>
...
function PqSpend({ cfg, wallet, ethUsd }: {
  cfg: ChainConfig; wallet: Wallet; ethUsd: number | null;
}) {
  ...
  const [spendState, setSpendState] = useState<Record<string, {
    dest: string; amount: string;
    step?: string | undefined; result?: string | undefined; error?: string | undefined;
  }>>({});
  ...
  if (!dep) {
    return (
      <div>
        ...
        <Note kind="warn">
          {cfg.key === 'anvil'
            ? <>No dev deployment found — start the dev chain first: <code>npm run chain</code> (it deploys the EntryPoint, verifier, factory, and registry, and starts the blinded-key signer service).</>
            : <>No {cfg.label} deployment found. Deploy the contracts (<code>npm run deploy:sepolia</code>) and run the signer service (<code>npm run signer</code>) first; both write the deployment file this tab reads.</>}
        </Note>
      </div>
    );
  }
  ...
  const receive = async () => {
    setError(null);
    const walletClient = wallet.clientFor(cfg);
    if (!walletClient?.account || !demoKem) { setError('Wallet not ready.'); return; }
    setBusy('receive');
    try {
      const publicClient = publicClientFor(cfg);
      ...
      const account = await publicClient.readContract({
        address: dep.factory, abi: FACTORY_ABI, functionName: 'getAccountAddress',
        args: pkArgs,
      });
      const value = parseEther(receiveAmount as `${number}`);
      const payTx = await walletClient.sendTransaction({
        account: walletClient.account, chain: cfg.chain, to: account, value });
      await publicClient.waitForTransactionReceipt({ hash: payTx });
      const annTx = await walletClient.writeContract({
        account: walletClient.account, chain: cfg.chain,
        address: cfg.announcer, abi: ANNOUNCER_ABI, functionName: 'announce',
  ...
  const scan = async () => {
    ...
    try {
      ...
      for (const ann of announcements) {
        ...
        found.push({
          ann, ssHex, pkArgs, account,
          balance, deployed: (code?.length ?? 0) > 2,
        });
      }
      setHits(found);
  ...
  const spend = async (hit: PqHit) => {
    ...
    const walletClient = wallet.clientFor(cfg);
    if (!walletClient?.account) { patch({ error: 'Wallet not ready.' }); return; }
    try {
      const dest = getAddress(st.dest.trim());
      const value = parseEther(st.amount as `${number}`);
      const publicClient = publicClientFor(cfg);
      ...
      if (!hit.deployed) {
        ...
        const h = await walletClient.writeContract({
          account: walletClient.account, chain: cfg.chain,
          address: dep.factory, abi: FACTORY_ABI, functionName: 'createAccount',
      ...
      const op = await buildSpendUserOp(
        publicClient, dep.entryPoint, hit.account, dest, value);
      ensureFunded(
        await publicClient.getBalance({ address: hit.account }), value, requiredPrefund(op));
      const userOpHash = await publicClient.readContract({
        address: dep.entryPoint, abi: ENTRYPOINT_ABI,
      ...
      const opsTx = await walletClient.writeContract({
        account: walletClient.account, chain: cfg.chain,
        address: dep.entryPoint, abi: ENTRYPOINT_ABI, functionName: 'handleOps',
  ...
  return (
    <div>
      <h3>PQ route — ERC-4337 account, on-chain ML-DSA verify</h3>
      <p className="fine">
        Level-2 (ZKNOX Dilithium2) demo profile — the parameter set of the deployed
        verifier. The announced stealth address is the counterfactual CREATE2 address
        of a <code>Stealth7913Account4337</code> whose 40-byte signer is
        {' '}<code>verifier ‖ PKContract</code>; blinded signing runs in the local
        Python service. EntryPoint {txLink(dep.entryPoint, cfg)}, factory {txLink(dep.factory, cfg)}.
      </p>
      <div className="row">
        <label className="field narrow" style={{ margin: 0 }}>
          <span>Receive amount (ETH){usd(Number(receiveAmount), ethUsd)}</span>
        ...
        return (
          <div className="hit" key={h.ann.txHash}>
            ...
            <div className="row" style={{ margin: '6px 0 0' }}>
              ...
              <input placeholder="amount ETH" inputMode="decimal" value={st.amount} style={{ width: 110 }}
                onChange={(e) => setSpendState((s) => ({ ...s, [h.ann.txHash]: { ...st, amount: e.target.value } }))} />
              <button disabled={!!st.step || !st.dest.trim() || !st.amount} onClick={() => spend(h)}>
...
function HybridSepoliaSpend({ cfg, wallet, ethUsd, dep }: {
  cfg: ChainConfig; wallet: Wallet; ethUsd: number | null; dep: DevDeployment;
}) {
  ...
  const [apiKey, setApiKey] = useState(() => {
    try { return localStorage.getItem('pq-stealth.pimlico-key') ?? ''; } catch { return ''; }
  });
  const [spendState, setSpendState] = useState<Record<string, {
    dest: string; amount: string;
    step?: string | undefined; result?: string | undefined; error?: string | undefined;
  ...
  const setKey = (k: string) => {
    setApiKey(k);
    try { localStorage.setItem('pq-stealth.pimlico-key', k); } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`${dep.signerService}/hybrid/derive`, { method: 'POST',
        ...
        const acct = await publicClientFor(cfg).readContract({
          address: dep.factory, abi: ZKNOX_FACTORY_ABI, functionName: 'getAddress',
  ...
  const receive = async () => {
    ...
    setError(null);
    const walletClient = wallet.clientFor(cfg);
    if (!walletClient?.account) { setError('Connect a browser wallet.'); return; }
    setBusy('receive');
    try {
      const publicClient = publicClientFor(cfg);
      const value = parseEther(receiveAmount as `${number}`);
  ...
  const spend = async (hit: HybridHit) => {
    ...
    patch({ error: undefined, result: undefined });
    const walletClient = wallet.clientFor(cfg);
    if (!walletClient?.account) { patch({ error: 'Connect a browser wallet.' }); return; }
    try {
      const dest = getAddress(st.dest.trim());
      const value = parseEther(st.amount as `${number}`);
      const publicClient = publicClientFor(cfg);
      ...
      if (!hit.deployed) {
        ...
        const h = await walletClient.writeContract({
          account: walletClient.account, chain: cfg.chain,
          address: dep.factory, abi: ZKNOX_FACTORY_ABI, functionName: 'createAccount',
          args: [preQuantumDemoKey().address, demo.publicKeyData], gas: 10_000_000n });
        await publicClient.waitForTransactionReceipt({ hash: h });
      }

      // the blinded ML-DSA half always comes from the signer service
      const signHybrid = async (userOpHash: Hex): Promise<Hex> => {
        const sr = await fetch(`${dep.signerService}/hybrid/sign`, { method: 'POST',
      ...
      if (usePimlico) {
        ...
        const nonce = await publicClient.readContract({
          address: dep.entryPoint, abi: HYBRID_ENTRYPOINT_ABI,
          functionName: 'getNonce', args: [hit.account, 0n] });
        const callData = encodeFunctionData({
          abi: HYBRID_ACCOUNT_ABI, functionName: 'execute', args: [dest, value, '0x'] });
        const res = await submitSponsoredUserOp({
          url: pimlicoUrl(cfg.chain.id, apiKey.trim()), chainId: cfg.chain.id,
          sender: hit.account, nonce, callData,
        ...
        patch({ step: undefined,
          result: `Spent ${st.amount} ETH -> ${dest}, gas sponsored by the Pimlico paymaster `
      ...
      ensureFunded(
        await publicClient.getBalance({ address: hit.account }), value, requiredPrefundHybrid(op));
      const userOpHash = await publicClient.readContract({
        address: dep.entryPoint, abi: HYBRID_ENTRYPOINT_ABI,
        functionName: 'getUserOpHash', args: [op] });
      ...
      const opsTx = await walletClient.writeContract({
        account: walletClient.account, chain: cfg.chain,
        address: dep.entryPoint, abi: HYBRID_ENTRYPOINT_ABI, functionName: 'handleOps',
        args: [[op], walletClient.account.address], gas: 11_000_000n });
      const rcpt = await publicClient.waitForTransactionReceipt({ hash: opsTx });
      if (rcpt.status !== 'success') throw new Error('handleOps reverted');
      patch({ step: undefined,
  ...
  return (
    <div>
      ...
      <p className="fine">
        ...
        <a href="https://github.com/ethereum/kohaku/tree/master/examples/pq-account"
          target="_blank" rel="noreferrer">ethereum/kohaku</a>): the account is a hybrid
        ECDSA + ML-DSA (level-2) signer, so the userOp carries both a classical signature
        and the blinded ML-DSA signature verified on-chain. Verifier {txLink(dep.verifier, cfg)},
        factory {txLink(dep.factory, cfg)}. Fixed demo identity.
      </p>
      {account && (
        <p className="fine">Demo stealth account: <AddressChip address={account} explorer={cfg.explorer} /></p>
      )}
      <label className="field">
        <span>
          Pimlico API key (optional) — {usePimlico
            ? 'gasless: the paymaster sponsors gas and bundles the op'
            : 'without it, the op self-bundles and the account pays its own gas'}
        </span>
        <input type="password" placeholder="pim_… (stored in this browser only)"
      ...
      <div className="row">
        ...
        <button disabled={busy !== null || !account} onClick={receive}>
          {busy === 'receive' ? 'Receiving…' : 'Receive demo payment'}
        </button>
        <button disabled={busy !== null || !account} onClick={scan}>
          {busy === 'scan' ? (scanInfo ?? 'Scanning…') : 'Scan for payments'}
        </button>
      </div>
      {error && <Note kind="error">{error}</Note>}
      {hits && (
        <Note kind={hits.length ? 'ok' : 'info'}>
          {hits.length ? `${hits.length} spendable payment${hits.length === 1 ? '' : 's'} found`
            : 'No payments found'}{scanInfo ? ` — ${scanInfo}` : ''}
        </Note>
      )}
      {hits?.map((h) => {
