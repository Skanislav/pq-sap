...
export function ScanTab({
  ...
  const scan = async () => {
    ...
    try {
      ...
      const { announcements, fromBlock, toBlock: latest } = await fetchAnnouncements(cfg, publicClient, setStatus)

      // 2. decapsulate + check in the worker
      setStatus(`Scanning ${announcements.length} announcements…`)
      const worker = new Worker(new URL('../lib/scan-worker.ts', import.meta.url), { type: 'module' })
      workerRef.current = worker
      const result = await new Promise<{ hits: ScanHit[]; elapsedMs: number }>((resolve, reject) => {
        worker.onmessage = (ev: MessageEvent<ScanResponse>) => {
          const m = ev.data
          if (m.type === 'progress') setStatus(`Scanning… ${m.done}/${m.total}`)
        ...
        worker.postMessage({
          metaAddress,
          kemDk,
          announcements: announcements.map(({ stealthAddress, ephemeralPubKey, viewTag }) => ({
            stealthAddress,
            ephemeralPubKey,
            viewTag,
          })),
      ...
      for (const h of result.hits) {
        ...
        try {
          ...
          /* balance is decoration; the detection already happened */
        }
        found.push({
          announcement: ann,
          sharedSecret: h.sharedSecret,
          stealthPk: h.stealthPk,
          balanceWei,
        })
      }
      setHits(found)
  ...
  return (
    <section>
      ...
      <p className="lede">
        The scanner decapsulates every announcement's ML-KEM ciphertext with your viewing key, filters on the 1-byte
        view tag, and re-derives the blinded stealth key for candidates. Spending keys are never touched.
      </p>
      ...
      <label className="field">
        ...
        <textarea
          ...
          placeholder="0x…"
          value={dkHex}
          onChange={(e) => setDkHex(e.target.value)}
        />
      </label>

      <div className="row">
        <button type="button" disabled={scanning || !metaHex.trim() || !dkHex.trim()} onClick={scan}>
      ...
      {stats && hits && (
        <>
          <Note kind={hits.length ? 'ok' : 'info'}>
            {hits.length ? `Found ${hits.length} payment${hits.length === 1 ? '' : 's'}` : 'No payments found'} in{' '}
            {stats.announcements} announcement{stats.announcements === 1 ? '' : 's'} (blocks{' '}
            {stats.fromBlock.toString()}–{stats.toBlock.toString()}, {stats.elapsedMs.toFixed(1)} ms
            ...
            return (
              <div className="hit" key={addr + h.announcement.txHash}>
                ...
                <div>
                  {h.balanceWei != null ? (
                    <strong>
                      {formatEther(h.balanceWei)} ETH{usd(eth!, ethUsd)}
                    </strong>
                  ) : (
                ...
                <div className="fine">
                  ...
                  {cfg.explorer ? (
                    <a href={`${cfg.explorer}/tx/${h.announcement.txHash}`} target="_blank" rel="noreferrer">
                      {h.announcement.txHash.slice(0, 10)}…
                    </a>
                  ) : (
                    <code>{h.announcement.txHash.slice(0, 10)}…</code>
                  )}
                  {' · '}blinded stealth pk derived ({h.stealthPk.length} B) — spendable via the ERC-7913 account path
                </div>
              </div>
            )
          })}
