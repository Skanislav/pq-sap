...
import {
  encodeAbiParameters, encodeFunctionData, keccak256, parseAbi, stringToHex,
  toHex as viemToHex, concatHex,
...
export async function buildSpendUserOp(
  ...
  const nonce = await publicClient.readContract({
    address: entryPoint, abi: ENTRYPOINT_ABI, functionName: 'getNonce', args: [account, 0n],
  });
  const gasPrice = await publicClient.getGasPrice();
  const maxFee = gasPrice * 2n < 1_500_000_000n ? 1_500_000_000n : gasPrice * 2n;
  return {
    sender: account, nonce, initCode: '0x',
...
export async function packHybridSignature(
  userOpHash: Hex, blindedSig: Hex,
): Promise<Hex> {
