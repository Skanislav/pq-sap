...
import {
  concatHex, encodeAbiParameters, keccak256, numberToHex, toHex,
...
export function computeUserOpHash(op: UnpackedUserOp, chainId: number): Hex {
  ...
  const packed = keccak256(encodeAbiParameters(
    ...
    [
      op.sender, op.nonce, keccak256('0x'), keccak256(op.callData),
      accountGasLimits, op.preVerificationGas, gasFees, keccak256(packPaymasterAndData(op)),
    ]));
...
export async function getUserOpGasPrice(
  ...
  const r = await rpc<{ standard: { maxFeePerGas: Hex; maxPriorityFeePerGas: Hex } }>(
    url, 'pimlico_getUserOperationGasPrice', []);
  return {
...
export async function sponsorUserOp(
  url: string, op: UnpackedUserOp, context?: Record<string, unknown>,
  ...
  return {
    ...
    paymasterVerificationGasLimit: r.paymasterVerificationGasLimit
      ? BigInt(r.paymasterVerificationGasLimit) : op.paymasterVerificationGasLimit,
    paymasterPostOpGasLimit: r.paymasterPostOpGasLimit
      ? BigInt(r.paymasterPostOpGasLimit) : op.paymasterPostOpGasLimit,
    callGasLimit: r.callGasLimit ? BigInt(r.callGasLimit) : op.callGasLimit,
    verificationGasLimit: r.verificationGasLimit
      ? BigInt(r.verificationGasLimit) : op.verificationGasLimit,
    preVerificationGas: r.preVerificationGas
      ? BigInt(r.preVerificationGas) : op.preVerificationGas,
  };
}
...
export async function submitSponsoredUserOp(args: {
  ...
  sender: Address; nonce: bigint; callData: Hex;
  dummySignature: Hex;
  ...
  return {
    userOpHash: sent, txHash: receipt.receipt.transactionHash,
    gasUsed: receipt.actualGasUsed, success: receipt.success,
  };
}
...
export async function waitForUserOpReceipt(
  url: string, userOpHash: Hex, opts: { tries?: number; delayMs?: number } = {},
  ...
  for (let i = 0; i < tries; i++) {
    ...
    if (r) return {
      success: r.success, actualGasUsed: BigInt(r.actualGasUsed), receipt: r.receipt };
