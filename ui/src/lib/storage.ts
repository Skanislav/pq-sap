...
export function saveSeeds(s: RecipientSeeds): void {
  try {
    localStorage.setItem(KEY, JSON.stringify(
      { zeta: toHex(s.zeta), kemD: toHex(s.kemD), kemZ: toHex(s.kemZ) }));
  } catch { /* private windows etc. — keys stay in-memory only */ }
}
...
export function loadClassicalSeeds(): ClassicalSeeds | null {
  try {
    ...
    return {
      spendSeed: fromHex(j.spendSeed), kemD: fromHex(j.kemD), kemZ: fromHex(j.kemZ) };
  } catch {
    return null;
  }
}
...
export function saveClassicalSeeds(s: ClassicalSeeds): void {
  try {
    localStorage.setItem(CLASSICAL_KEY, JSON.stringify({
      spendSeed: toHex(s.spendSeed), kemD: toHex(s.kemD), kemZ: toHex(s.kemZ) }));
  } catch { /* private windows etc. */ }
}

export function clearClassicalSeeds(): void {
  try { localStorage.removeItem(CLASSICAL_KEY); } catch { /* ignore */ }
}
