...
import {
  decodeAbiParameters, encodeFunctionData, parseAbi, toHex as viemToHex,
...
export async function fetchDeployment(
  chainKey: 'anvil' | 'sepolia',
): Promise<DevDeployment | null> {
...
export const deriveSpendable = (service: string, ss: Hex) =>
  callService<DerivedSpendable>(service, '/derive', { ss });
...
export async function buildSpendUserOp(
  publicClient: PublicClient, entryPoint: Address,
  account: Address, dest: Address, value: bigint,
): Promise<UserOp> {
