...
import {
  createWalletClient, custom, http, numberToHex,
  type Address, type EIP1193Provider, type WalletClient,
} from 'viem';
...
export function useWallet(): Wallet {
  ...
  const switchChain = useCallback(async (cfg: ChainConfig) => {
    ...
    try {
      await eth.request({
        method: 'wallet_switchEthereumChain', params: [{ chainId: hexId }] });
    } catch (e) {
  ...
  const clientFor = useCallback((cfg: ChainConfig): WalletClient | null => {
    if (mode === 'dev') {
      if (cfg.key !== 'anvil') return null;
      return createWalletClient({
        chain: cfg.chain, transport: http(cfg.rpcUrl), account: devAccount });
    }
    if (!window.ethereum || !address) return null;
    return createWalletClient({
      chain: cfg.chain, transport: custom(window.ethereum), account: address });
  }, [mode, address]);

  const gate = useCallback((cfg: ChainConfig): 'connect' | 'switch' | 'ready' => {
  ...
  return useMemo(() => ({
    mode, setMode, hasInjected,
    address: mode === 'dev' ? devAccount.address : address,
    chainId, connecting, connect, switchChain, clientFor, gate,
