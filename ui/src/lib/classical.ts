...
export function deriveClassicalKeys(seeds: ClassicalSeeds): ClassicalKeys {
  ...
  const { publicKey: kemEk, secretKey: kemDk } =
    ml_kem768.keygen(concat(seeds.kemD, seeds.kemZ));
  const metaAddress = concat(Uint8Array.of(CLASSICAL_META_VERSION), spendPub, kemEk);
  if (metaAddress.length !== CLASSICAL_META_BYTES)
    throw new Error(`classical meta-address is ${metaAddress.length} B`);
...
export function deriveTweak(ss: Uint8Array): bigint {
  const seed = shake256(
    concat(new TextEncoder().encode(TWEAK_DOMAIN), ss), { dkLen: TWEAK_SEED_BYTES });
...
export function encodeCompactMeta(spendPub: Uint8Array, index: bigint): Uint8Array {
  ...
  for (let i = 0; i < 32; i++)
    out[33 + i] = parseInt(hex.slice(2 * i, 2 * i + 2), 16);
  return out;
}
...
export function checkClassicalAnnouncement(
  meta: ClassicalMeta, kemDk: Uint8Array, ann: AnnouncementData,
): Payment | null {
  let ss: Uint8Array;
  try {
