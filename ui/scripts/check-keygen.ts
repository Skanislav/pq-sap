...
for (const [name, rec] of Object.entries<{
  ...
  assert.equal(toHex(keys.metaAddress), rec.meta_address, `recipient ${name}: meta-address mismatch`)
  assert.equal(toHex(keys.kemDk), rec.kem_dk, `recipient ${name}: viewing key mismatch`)
...
for (const [name, rec] of Object.entries<{
  ...
  assert.equal(toHex(keys.metaAddress), rec.meta_address, `classical ${name}: meta-address mismatch`)
  assert.equal(toHex(keys.kemDk), rec.kem_dk, `classical ${name}: viewing key mismatch`)
  assert.equal(toHex(keys.seeds.spendSeed), rec.spend_priv, `classical ${name}: spend seed is the spending secret`)
...
for (const c of cv.cases as Array<{
  ...
  if (c.encaps_m) {
    ...
    const { cipherText, sharedSecret } = ml_kem768.encapsulate(meta.kemEk, fromHex(c.encaps_m))
    assert.equal(toHex(cipherText), c.announcement.ephemeral_pub_key, `${c.name}: ciphertext mismatch`)
    assert.equal(toHex(classicalViewTag(sharedSecret)), c.announcement.view_tag, `${c.name}: view tag mismatch`)
    const P = deriveStealthPubkey(meta.spendPub, sharedSecret)
    if (c.stealth_pub) assert.equal(toHex(P.toBytes(true)), c.stealth_pub, `${c.name}: stealth pubkey mismatch`)
    assert.equal(toHex(ethAddressOfPoint(P)), c.announcement.stealth_address, `${c.name}: stealth address mismatch`)
    ...
    assert.equal(
      toHex(ethAddressOfPoint(PfromPriv)),
      c.announcement.stealth_address,
  ...
  if (c.expect === 'valid_proof') {
    ...
    assert.ok(
      secp256k1.verify(rs, fromHex(c.challenge!), payment.stealthPk, { prehash: false }),
...
// and the spend pubkey it carries matches the full meta-address's
{
  ...
  for (const index of [0n, 1n, 42n, (1n << 255n) + 7n]) {
    ...
    assert.equal(toHex(decoded.spendPub), toHex(meta.spendPub), 'compact meta must carry the same spend pubkey inline')
    assert.ok(compact[0] === 0x02 || compact[0] === 0x03, 'version byte is the SEC1 parity prefix')
  }
