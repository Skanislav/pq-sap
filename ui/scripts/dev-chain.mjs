...
import { startSignerService, SIGNER_PORT, DEMO } from './signer-service.mjs';
...
const classicalVectors = JSON.parse(
  readFileSync(here('../../python/vectors/classical/v0/vectors.json'), 'utf8'));
...
for (let i = 0; ; i++) {
  try { await publicClient.getBlockNumber(); break; }
    ...
    if (i > 100) { console.error('anvil did not come up'); process.exit(1); }
...
async function deploy(art, args = []) {
  const hash = await walletClient.deployContract({
    abi: art.abi, bytecode: art.bytecode.object, args, gas: 30_000_000n });
...
console.log(`deployed  entryPoint ${entryPoint}\n          verifier   ${verifier}\n          factory    ${factory}\n          registry   ${registry}`);
...
// 65-byte compact meta-address (spend_pub || 0) resolves out of the box
{
  ...
  const h = await walletClient.writeContract({
    address: registry, abi: registryArt.abi, functionName: 'register',
    args: [ekHex] });
...
async function announce(stealthAddress, ephemeralPubKey, viewTag) {
  const h = await walletClient.writeContract({
    address: announcer, abi: announcerArt.abi, functionName: 'announce',
...
const [aHatEnc, tr, t1Enc] = decodeAbiParameters(
  [{ type: 'bytes' }, { type: 'bytes' }, { type: 'bytes' }], fixture.public_key_data);
...
const spendableAccount = await publicClient.readContract({
  address: factory, abi: factoryArt.abi, functionName: 'getAccountAddress',
  args: [aHat, tr, t1],
});
...
const deployment = {
  mode: 'stealth7913',
  chainId: 31337, announcer, entryPoint, verifier, factory, registry,
  signerService: `http://127.0.0.1:${SIGNER_PORT}`,
  demo: DEMO,
};
mkdirSync(here('../public'), { recursive: true });
writeFileSync(here('../public/dev-deployment.json'),
