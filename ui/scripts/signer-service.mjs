...
const PYTHONREF = process.env.ZKNOX_PYTHONREF
  ?? here('../../js-client/contracts/lib/ETHDILITHIUM/pythonref');
...
export function startSignerService(port = SIGNER_PORT) {
  ...
  function runHelper(ss, challenge) {
    return new Promise((resolve, reject) => {
      ...
      const args = [HELPER, '--pythonref', PYTHONREF, '--zeta', DEMO.zeta,
        '--ss', ss, '-o', out];
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === 'GET') { res.writeHead(200); return res.end('{"ok":true}'); }
    let body = '';
      ...
      try {
        ...
        if (req.url === '/hybrid/derive') {
          ...
          return json({
            ...
            kem_ct: hybridDemo.kem_ct, view_tag: hybridDemo.view_tag,
