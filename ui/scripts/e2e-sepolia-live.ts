...
import {
  ...
  encodeFunctionData, hexToBytes, getAddress,
  type Hex, type Address,
} from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import { sepolia } from 'viem/chains';

import { SEPOLIA, ANNOUNCER_ABI, SCHEME_ID } from '../../js-client/src/sepolia.ts';
import {
  ENTRYPOINT_ABI, ACCOUNT_ABI, ZKNOX_FACTORY_ABI, preQuantumDemoKey,
  packHybridSignature, dummyHybridSignature,
} from '../src/lib/spendHybrid.ts';
import { KOHAKU_SEPOLIA } from '../src/lib/kohaku.ts';
import {
  pimlicoUrl, submitSponsoredUserOp, computeUserOpHash,
} from '../src/lib/pimlico.ts';
...
try { process.loadEnvFile(here('../../.env')); } catch { /* may already be in env */ }
...
try {
  ...
  const nonce = await publicClient.readContract({
    ...
    functionName: 'getNonce', args: [account, 0n] }) as bigint;
  const callData = encodeFunctionData({
    abi: ACCOUNT_ABI, functionName: 'execute', args: [recipient, spendValue, '0x'] });
  ...
  const sign = async (userOpHash: Hex): Promise<Hex> => {
    ...
    assert.equal(blindedLen, MLDSA44_SIG_BYTES,
  ...
  const res = await submitSponsoredUserOp({
    url, chainId, sender: account, nonce, callData, dummySignature, sign,
  ...
  assert.equal(accBeforeSpend - accAfter, spendValue,
