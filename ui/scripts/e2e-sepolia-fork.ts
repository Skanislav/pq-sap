...
import { SEPOLIA, ANNOUNCER_ABI, FACTORY_ABI, SCHEME_ID } from '../../js-client/src/sepolia.ts';
import { startSepoliaFork } from '../../js-client/test/util/anvil.ts';
import {
  ENTRYPOINT_ABI, ZKNOX_FACTORY_ABI, preQuantumDemoKey,
  buildSpendUserOp, packHybridSignature, requiredPrefund,
...
import { startSignerService, SIGNER_PORT } from './signer-service.mjs';
...
try {
  ...
  assert.equal(SEPOLIA.zknoxMldsaK1Factory, KOHAKU_SEPOLIA.mldsaK1Factory,
  ...
  const account = await publicClient.readContract({
    address: KOHAKU_SEPOLIA.mldsaK1Factory, abi: ZKNOX_FACTORY_ABI,
    functionName: 'getAddress', args: [preQ.address, demo.public_key_data as Hex],
  }) as Address;
  ...
  const annHash = await wallet.writeContract({
    address: SEPOLIA.erc5564Announcer, abi: ANNOUNCER_ABI, functionName: 'announce',
  ...
  const op = await buildSpendUserOp(
    publicClient, KOHAKU_SEPOLIA.entryPoint, account, recipient, spendValue);
  ...
  const createHash = await wallet.writeContract({
    address: KOHAKU_SEPOLIA.mldsaK1Factory, abi: ZKNOX_FACTORY_ABI,
    functionName: 'createAccount', args: [preQ.address, demo.public_key_data as Hex],
    gas: 10_000_000n });
  ...
  const userOpHash = await publicClient.readContract({
    address: KOHAKU_SEPOLIA.entryPoint, abi: ENTRYPOINT_ABI,
  ...
  assert.equal(offchain, userOpHash,
    'off-chain computeUserOpHash (Pimlico path) must match EntryPoint.getUserOpHash');
  ...
  // offline) rather than discovering an AA24 after funding real ETH.
  {
    ...
    const synthUnpacked = {
      sender: op.sender, nonce: 7n, callData: op.callData,
      callGasLimit: 400_000n, verificationGasLimit: 9_000_000n,
    ...
    const synthPacked = {
      sender: synthUnpacked.sender, nonce: synthUnpacked.nonce, initCode: '0x' as Hex,
    ...
    const onchainPm = await publicClient.readContract({
      address: KOHAKU_SEPOLIA.entryPoint, abi: ENTRYPOINT_ABI,
      functionName: 'getUserOpHash', args: [synthPacked] }) as Hex;
    const offchainPm = computeUserOpHash(synthUnpacked, sepolia.id);
    assert.equal(offchainPm, onchainPm,
      'sponsored-op (paymasterAndData) hash parity must hold');
    console.log('    sponsored-op paymasterAndData hash parity ✓');
  }
  const { sig: blindedSig } = await postJson<{ sig: Hex }>(
    '/hybrid/sign', { challenge: userOpHash });
  ...
  const opsHash = await wallet.writeContract({
    address: KOHAKU_SEPOLIA.entryPoint, abi: ENTRYPOINT_ABI, functionName: 'handleOps',
    args: [[op], eoa.address], gas: 11_000_000n });
