...
interface RawAnnouncement extends AnnouncementData {
  txHash: string
}
...
// ---------------------------------------------------------------------------
{
  ...
  const keys = deriveMetaAddress({
    zeta: fromHex(seeds.zeta),
    kemD: fromHex(seeds.kem_d),
    kemZ: fromHex(seeds.kem_z),
  })
  ...
  const stealthAddress = getAddress(toHex(stealthAddressOf(deriveStealthPk(meta.rho, meta.t, sharedSecret))))
  ...
  h = await walletClient.writeContract({
    ...
    args: [SCHEME_ID, stealthAddress, toHex(cipherText) as Hex, toHex(computeViewTag(sharedSecret)) as Hex],
  })
  await publicClient.waitForTransactionReceipt({ hash: h })
...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(stealthAccount.address, stealthAddr, 'derived key must control the stealth EOA')
...
// ---------------------------------------------------------------------------
{
  ...
  const op = await buildSpendUserOp(publicClient, deployment.entryPoint, hit.account, dest, parseEther('0.2'))
  ...
  const destAfter = await publicClient.getBalance({ address: dest })
  assert.equal(destAfter - destBefore, parseEther('0.2'), 'destination must receive the spent value')
  console.log(
    `pq SPEND: 0.2 ETH via EntryPoint.handleOps, ${opsRcpt.gasUsed} gas ` + '(incl. on-chain blinded ML-DSA verify) ✓',
  )
}
