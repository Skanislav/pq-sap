...
**Stretch goal:** measure the speed, gas costs and end-to-end implementation of a stealth-address protocol using ERC-4337 with post-quantum key exchange and a zero-knowledge ownership proof, for cheaper spending during the transition (per pq.ethereum.org). This is not part of the core deliverable.
...
* Security analysis in pq is quite complicated topic and proving that something is safe now does not mean that no new attack would be discovered in NIST approved list. We will try to make the best effort by relying on Lean4 proofs and existing academic research.
...
As a base reference we'll use the paper published by the [folks from Serbia](https://eprint.iacr.org/2025/112) and take their reference implementation (`0x3327/pq-sap`, Rust) as the starting point. What it doesn't have and we'll complement is a real ERC-5564 scheme ID, a key derivation that lands on a standardized signature scheme, conformance vectors, and execution benchmarks. The full flow in Python pseudocode.

```python
# --- Setup (recipient) -------------------------------------------------
sk_spend, K = MLDSA.keygen()      # spending keypair (ML-DSA master; K keeps t at full precision)
v, V = MLKEM768.keygen()          # viewing keypair (ML-KEM)
...
stealth_address = derive_stealth_address(K, S)   # construction A: blind K with a fresh (s', e') from S
...
# --- Spend (recipient only; requires spending key sk_spend) ------------
p = derive_stealth_key(sk_spend, S)   # blinded one-time key (widened bound beta' = tau*2*eta)
assert MLDSA.verify(pubkey(p), sign(p, msg), msg)   # stock FIPS 204 verifier
# or prove the ownership relation A*s + e = t in zero knowledge instead of signing
...
proof = prove_possession(p, challenge)   # sign over ctx = "pop" || stealth_address
assert verify_possession(stealth_address, challenge, proof)
```

**Notes:**

The viewing key `v` alone detects and views payments; spending needs `sk_spend` too. Standard viewing/spending separation.
...
1. An ERC draft with a declared ERC-5564 scheme ID for the post-quantum scheme.
...
- [Iván Mañús](https://github.com/ivanmmurciaua) contributed to the classical-spend hybrid design and cross-client validation work.
