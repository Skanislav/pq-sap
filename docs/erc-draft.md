...
The target account model is the protocol-native one: [EIP-8141](./eip-8141.md) frame transactions (type `0x06`, scheduled for inclusion in the Hegotá network upgrade) make a stealth account an address with code, whose VERIFY frame runs arbitrary validation logic as the sender. Under this model the stealth address announced by the sender is the counterfactual frame-transaction account address; the first spend deploys the account and verifies the post-quantum signature in a single native transaction (a `[deploy, self_verify]` validation prefix), the signature travels in the transaction's signatures array under the `ARBITRARY` scheme, and gas sponsorship is a native paymaster frame rather than external relay infrastructure. The verifier contracts described below are unchanged  -  they become libraries called from the account's VERIFY code rather than ERC-7913 signer targets.

Until frame transactions are live (and on chains that do not adopt them), on-chain verification of blinded-key signatures is expressed with the [ERC-7913](./eip-7913.md) signer encoding: the spend key is represented as the byte string `verifier || key`, where `verifier` is a stateless signature-verifier contract shared by all accounts and `key` identifies the stealth public key. The `key` field can be the raw 1,952-byte stealth public key (a fully stateless verifier) or a 20-byte pointer to deployed key storage (the pattern used by existing on-chain ML-DSA verifiers, where the expanded key is written once and the signer string is 40 bytes).
...
**Why additive blinding (construction A).** It preserves the ERC-5564 property that the sender computes the address, which sender-names-the-address designs require, and it lands on a *standard* ML-DSA public key, so every deployed FIPS 204 verifier accepts the resulting signatures. The alternative (construction B) — deriving a fresh standardized keypair from the shared secret — uses only standardized operations but breaks sender-side address computation and is documented only as a separate account-transfer protocol (D-021); it does not share this scheme ID.

**Why a code-carrying account, and why ERC-7913 in the interim.** The spend design targets [EIP-8141](./eip-8141.md) frame transactions because they deliver exactly what stealth spending needs at the protocol level: counterfactual accounts that originate their own transactions, arbitrary validation code (so any post-quantum verifier is admissible without a new transaction type per scheme), and native gas sponsorship (so a fresh stealth account never funds gas from a linkable source). One constraint is common to both eras: every stealth address carries a distinct blinded key, so any design that bakes the key into deployment code pays per-account deployment and, for post-quantum key sizes, collides with the EIP-3860 initcode cap (measured: an account embedding two ML-DSA keys exceeds the 49,152-byte cap and cannot deploy); keys therefore live in account state or behind pointers, never in initcode or deploy frames. Until frame transactions ship, an [ERC-7913](./eip-7913.md) signer expresses the same content: one shared verifier serves every stealth account, the account stores only `verifier || key` (40 bytes in the pointer form), and the encoding itself adds no verification cost  -  ERC-7913 is an interface, not an accelerator; the underlying signature check still dominates.
...
**Spending today and under frame transactions.** Value at these stealth addresses is spendable on-chain today only via account-abstraction routes (e.g. an ERC-4337 account whose address is the counterfactual CREATE2 address of the derived key, demonstrated on Sepolia with a deployed level-2 Dilithium verifier and an ECDSA co-signer). The [ERC-7913](./eip-7913.md) signer encoding above standardizes that route, but does not reduce its cost: direct on-chain ML-DSA verification remains in the millions of gas until protocol-level support (vector-math precompiles or native post-quantum signatures) lands. [EIP-8141](./eip-8141.md) frame transactions remove the off-chain infrastructure (EntryPoint, bundlers, external paymasters) but not the verification cost, and they introduce a mempool constraint this scheme must be honest about: public-mempool admission simulates the validation prefix under a bounded verification budget (`MAX_VERIFY_GAS`, 100,000 execution gas as specified), which no measured post-quantum verifier fits today  -  a hash-based SPHINCS+ "+C" verification is closest at roughly twice the budget, and ML-DSA exceeds it by two orders of magnitude. Post-quantum frame-transaction spends therefore ride block-builder or private-channel inclusion until either the budget accommodates them or a protocol-validated post-quantum signature scheme identifier exists; the latter is the durable fix. Wallets MUST make these limitations clear before letting users receive value under this scheme.
