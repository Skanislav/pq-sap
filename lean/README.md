...
`MultiRecipient` → `KEMAnonymity` → `ConstructionA` → `SharedSecretHiding` →
`AnonymityFromSPR` → `MLKEM` → `Ownership` → `Soundness` → `Controls`. `Demo` is a runnable toy
...
Fifteen content modules plus the axiom audit and the root.
