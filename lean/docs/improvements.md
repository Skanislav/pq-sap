...
Open: #7, #9, #16 (headers), plus the remaining halves of the follow-ups above
(identical-until-bad and the `Pr[bad]` bound in the ROM; the on-paper
compositions of the `n`-recipient and `q`-challenge bounds with
`unlinkAdvantage_ofKEMFull_le`; upstream VCVio
