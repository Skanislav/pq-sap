...
`simulatorGap` (encoding regularity), #9(b)'s lazy-RO switching lemma and the
`q_H` guessing step, `BlindingROM`'s identification with `auxKeyIndependence`
and its identical-until-bad step, `MultiRecipient`'s two never-fails
hypotheses at the concrete scheme; the upstream VCVio asks (`DecidableEq`
