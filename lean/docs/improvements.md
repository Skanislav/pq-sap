...
| 3 | Done (model + sanity lemma + reduction adversary) — `ConstructionA.lean`; `auxKeyIndependence_eq_zero_of_pk_independent`, `stealthAddr_eq_blinded_pk`, seeded-MLWE `mlweAdvOfUnlinkAdv`, `idealAux_indep_of_t`. **Correction to the issue text:** the blinding term is not bounded by MLWE alone — `rho` sits outside the mask, so closing it needs the address hash as a random oracle (documented; follow-up below). |
| 3-follow-up | Partial — `BlindingROM.lean`: `hashAddr ∘ pack` as VCVio's lazily sampled `randomOracle` over `unifSpec + (Bytes →ₒ Addr)` (`romImpl`, mirroring VCVio's `prfIdealQueryImpl`), the ROM blinding game (`blindTraceRO`/`blindGameRO`/`blindingAdvantageRO`), and three theorems: `run_hashAddrRO_empty` (from the empty cache the announced address is uniform **whatever** was hashed — the step the mask cannot supply, and the one that kills the `rho` dependence `idealAux_indep_of_t` had to share), `blindGameRO_eq` (the branches are the same computation from caches differing at one point) and `blindingAdvantageRO_eq_zero_of_no_query`. **Round 4: (b) is proved (`blindingAdvantageRO_le_blindBadProb`, via `ROMUpToBad`). Originally not proved, THREE items:** (a) `blindGameRO` is a standalone abstraction of the branch comparison — the KEM, ciphertext and tag derivation are dropped and `tg`/`rho`/`t` are free parameters — and is NOT identified with the two games `auxKeyIndependence` compares; that identification is not even stated in Lean, and needs the bind commutation plus `ExpandIsIdeal` that `ConstructionA`'s gap list already names. (b) the identical-until-bad step (`BoundedByBadQuery`) and (c) `Pr[bad] ≤ mlwe + q_H·β` (`BadQueryBounded`), both stated as `Prop` targets with the reduction `mlweAdvOfBlindAdvRO` type-checked. VCVio's `StateSeparating/IdenticalUntilBad.lean` wants `σ × Bool`-shaped handlers with an explicit bad flag, which `romImpl` is not; and the `Pr[bad]` bound needs a query bound plus min-entropy of `pack ∘ power2Round`, which is uninterpreted here. |
| 4 | Done (two recipients, `q` challenges) — `MultiUnlink.lean`: `UnlinkExpMulti`, `unlinkAdvantageMulti_le_sum` (`≤ ∑ k ∈ range q, unlinkAdvantage (hybridAdv k (q-k-1))`) and `unlinkAdvantageMulti_le_mul` (`≤ q * ε`), over the general `boolDistAdvantage_le_sum_hybrids`. **Loss factor `q`, linear** — the two recipients are fixed, so only the announcements are hybridised. |
| 4-follow-up | Done — `MultiRecipient.lean`: the `n`-recipient game (`UnlinkExpN`, the adversary NAMES the pair after seeing all `n` meta-addresses), `pairGuessAdv`, `unlinkAdvantageN_le_sum` and `unlinkAdvantageN_le_mul` (`≤ n·(n−1)·ε`). **Correction to the follow-up text:** it is NOT a conditioning argument — gating each reduction on the named pair makes the gated games disjoint, so their probabilities add (`sum_probOutput_unlinkBranchNAt`) and `\|∑ xⱼ\| ≤ ∑ \|xⱼ\|` finishes it; no uniform draw over pairs anywhere. The one new fact is exchangeability of `n` i.i.d. key generations (`evalDist_pubKeysN_embedPair`, from a `Fin.cons` induction). One explicit hypothesis: the adversary names two distinct recipients (the round-3 text also listed `keygen`/`announce` never failing; for `ProbComp` that is VCVio's `probFailure_of_liftM_PMF`, so those hypotheses were vacuous and were removed in round 4). Composition with the `q`-challenge hybrid and with `unlinkAdvantage_ofKEMFull_le` stays on paper. |
...
| 7 | Done in part (both lattice hops; two non-lattice terms named, not bounded) — `SPRTwoHop.lean`. **Correction to the issue text:** the result is NOT `SPR ≤ 2·MLWE`; it is the four-term `sprAdv ≤ primitiveIdealization + advantage keyHopProblem (keyHopAdv) + advantage ctHopProblem (ctHopAdv) + simulatorGap` (`sprAdv_le_two_hop_decomposition`, and `mlkem768_sprAdv_le_two_hop_decomposition` at `mlkem768KEM` / `mlkem768UniformCiphertext` with no instance hypotheses). Both `LearningWithErrors.Problem`s are **seeded** on `rho` (a matrix-sampled problem, as `kpke_ind_cpa_security` announces, is unusable by a reduction that must output a public key), and each hop's game gap is PROVED equal to its reduction adversary's `LearningWithErrors.advantage` — `game0/game1_{key,ct}HopProblem`, all `simp only [bind_assoc, pure_bind, …]` because the games draw challenge key → ciphertext → other key. New bridge `LearningWithErrors.advantage_eq_boolDistAdvantage`; new instances `instNeZeroMlkemModulus` / `instSampleableTypeRq` / `instSampleableTypeTq` (**fourth upstream ask**). Unbounded: `primitiveIdealization` (`G`/`PRF_η`/`SampleNTT` ideal — ML-KEM derives `s, e` from one `sigma` and the encryption coins from `G(m ‖ H(ek))`, i.e. from the challenge key) and `simulatorGap`, split by `simulatorGap_le` into `encodingRegularity` (lossy compress-then-encode of uniform ring elements vs uniform bytes; needs the `private` `byteEncode_size`) and `keyRestoration`. Implicit rejection provably irrelevant: no decapsulation appears in any game. **Not done:** the acceptance clause "rewrite `AnonymityFromSPR.lean`'s remaining-lattice-step paragraph" — that file was owned by another work item; needs reassigning. Details in `docs/spr-two-hop.md` § "What is now in Lean". |
...
| 12 | Done — `PqStealth/Axioms.lean`, 95 `#guard_msgs (whitespace := lax) in #print axioms` blocks; wrong list ⇒ build error (verified). |
...
with `auxKeyIndependence` and its `Pr[bad]` bound (its identical-until-bad step
is closed); the upstream VCVio asks (`DecidableEq`
...
**Acceptance.** `spendForgeryAdvantage_le_msis` (now named `spendForgeryAdvantage_le_augmentedSIS`, since the challenge is still the scheme's key distribution) stated against VCVio's
...
**Round 4.** The standard-model half is done without the ROM:
`hashedDH_le_ddh_add_es` bounds each `hashedDH` term by
`DiffieHellman.ddhDistAdvantage` of `ddhReductionDKSAP` plus
`EntropySmoothing.advantage` of `esReductionDKSAP` (VCVio's hashed-ElGamal
shape), so `dksap_unlinkAdvantage_le_ddh_add_es` is "DKSAP is unlinkable under
DDH + entropy smoothing of `h`" with explicit reductions.
