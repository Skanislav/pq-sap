...
— and the pair-guessing bound `unlinkAdvantageN ≤ ∑ over ordered pairs of
...
Assumed: `keygen` and `announce` never fail and the adversary names two DISTINCT
recipients — all three are explicit hypotheses of the headline theorems. See
...
the draw it displaces is where `keygen`'s totality is used. -/
theorem evalDist_pubKeysN_update {β : Type} (hkg : Pr[⊥ | S.keygen] = 0) :
  ...
  | succ n ih =>
      ...
      · intro k
        ...
        rw [hL, OracleComp.DeferredSampling.evalDist_bind_const_neverFails S.keygen hkg]
...
theorem evalDist_pubKeysN_embedPair {β : Type} (hkg : Pr[⊥ | S.keygen] = 0) {n : ℕ}
  ...
  have h3 : 𝒟[do let f ← S.pubKeysN n
    ...
    S.evalDist_pubKeysN_update hkg n j.1
      fun g => do let y ← S.keygen; k (Function.update g j.2 y.1)
  rw [h0, h1, h2, h3]
  exact S.evalDist_pubKeysN_update hkg n j.2 k

/-! ## The `n`-recipient game -/

/-- Challenge selection: after seeing all `n` published meta-addresses the
adversary names the ordered pair of recipients it wants to distinguish. -/
abbrev UnlinkChooseN (MetaPub : Type) (n : ℕ) :=
  (Fin n → MetaPub) → ProbComp (Fin n × Fin n)

/-- Guessing stage: all `n` meta-addresses, the named pair, and the challenge
announcement, which is addressed to one of the two named recipients. -/
abbrev UnlinkGuessN (MetaPub Announcement : Type) (n : ℕ) :=
  (Fin n → MetaPub) → Fin n × Fin n → Announcement → ProbComp Bool

variable {n : ℕ} (pick : UnlinkChooseN MetaPub n) (guess : UnlinkGuessN MetaPub Announcement n)
...
def unlinkBranchNAt (j : Fin n × Fin n) (b : Bool) (pks : Fin n → MetaPub) : ProbComp Bool := do
  let i ← pick pks
  let c ← S.announce (pks (if b then i.2 else i.1))
  if i = j then guess pks i c else pure false
...
def pairGuessAdv (j : Fin n × Fin n) : UnlinkAdv MetaPub Announcement :=
  fun pk0 pk1 c => do
    ...
    if i = j then guess (embedPair j f (pk0, pk1)) i c else pure false
...
theorem evalDist_unlinkBranch_pairGuessAdv (hkg : Pr[⊥ | S.keygen] = 0)
    (hann : ∀ pk : MetaPub, Pr[⊥ | S.announce pk] = 0)
  ...
  have hL : (S.unlinkSetup >>= S.unlinkBranch (S.pairGuessAdv pick guess j) b) =
      (do let a ← S.unlinkSetup
          ...
          if i = j then guess (embedPair j f a) i c else pure false) := rfl
  have hswap : 𝒟[do let a ← S.unlinkSetup
                    let c ← S.announce (if b then a.2 else a.1)
                    let f ← S.pubKeysN n
                    let i ← pick (embedPair j f a)
                    if i = j then guess (embedPair j f a) i c else pure false] =
      𝒟[do let a ← S.unlinkSetup
            let f ← S.pubKeysN n
            let c ← S.announce (if b then a.2 else a.1)
            let i ← pick (embedPair j f a)
  ...
  have hstep : 𝒟[do let a ← S.unlinkSetup
      ...
      𝒟[S.pubKeysN n >>= fun pks => do
            ...
            let i ← pick pks
            if i = j then guess pks i c else pure false] := by
    refine Eq.trans (congrArg _ (bind_congr fun a => bind_congr fun f => ?_))
      (S.evalDist_pubKeysN_embedPair hkg j fun pks => do
        let c ← S.announce (pks (if b then j.2 else j.1))
        let i ← pick pks
        if i = j then guess pks i c else pure false)
    cases b with
  ...
  rw [evalDist_bind_bind_swap (S.announce (pks (if b then j.2 else j.1))) (pick pks)
    fun c i => if i = j then guess pks i c else pure false]
  ...
  · simp only [if_neg hij]
    rw [OracleComp.DeferredSampling.evalDist_bind_const_neverFails _
        (hann (pks (if b then j.2 else j.1))) (pure false),
      OracleComp.DeferredSampling.evalDist_bind_const_neverFails _
        (hann (pks (if b then i.2 else i.1))) (pure false)]
...
theorem sum_probOutput_unlinkBranchNAt
    (hpick : ∀ (pks : Fin n → MetaPub), ∀ i ∈ support (pick pks), i.1 ≠ i.2) (b : Bool) :
  ...
  have hinner : ∀ pks : Fin n → MetaPub,
    ...
    · have hmem : i ∈ (Finset.univ.offDiag : Finset (Fin n × Fin n)) := by
        simp only [Finset.mem_offDiag, Finset.mem_univ, true_and]
        exact hi
      congr 1
      refine Finset.sum_eq_single_of_mem i hmem ?_ |>.trans ?_
      · intro j _ hji
...
theorem unlinkAdvantageN_le_sum (hkg : Pr[⊥ | S.keygen] = 0)
    (hann : ∀ pk : MetaPub, Pr[⊥ | S.announce pk] = 0)
    (hpick : ∀ (pks : Fin n → MetaPub), ∀ i ∈ support (pick pks), i.1 ≠ i.2) :
    S.unlinkAdvantageN pick guess ≤
  ...
  have hterm : ∀ j ∈ (Finset.univ.offDiag : Finset (Fin n × Fin n)),
    ...
    rw [S.unlinkAdvantage_eq_branchDistAdvantage, ProbComp.boolDistAdvantage,
      probOutput_congr rfl (S.evalDist_unlinkBranch_pairGuessAdv pick guess hkg hann hj.2.2 true),
      probOutput_congr rfl (S.evalDist_unlinkBranch_pairGuessAdv pick guess hkg hann hj.2.2 false)]
  have hsum : ∀ b : Bool,
...
is at most `n·(n−1)·ε`. -/
theorem unlinkAdvantageN_le_mul (hkg : Pr[⊥ | S.keygen] = 0)
    (hann : ∀ pk : MetaPub, Pr[⊥ | S.announce pk] = 0)
    (hpick : ∀ (pks : Fin n → MetaPub), ∀ i ∈ support (pick pks), i.1 ≠ i.2) {ε : ℝ}
    (h : ∀ j : Fin n × Fin n, S.unlinkAdvantage (S.pairGuessAdv pick guess j) ≤ ε) :
    S.unlinkAdvantageN pick guess ≤ (n * (n - 1) : ℕ) * ε := by
  refine le_trans (S.unlinkAdvantageN_le_sum pick guess hkg hann hpick) ?_
