...
:::info[Mirrored page]
Source: [`lean/README.md`](https://github.com/Skanislav/pq-sap/blob/main/lean/README.md). Edit the source file; this copy is regenerated by `pnpm sync` in `wiki/`.
:::
...
`ConstructionASecurity` → `Soundness` → `Controls`. `Demo` is a runnable toy
instance, so it is the cheapest way in.

The design essays live in [`docs/`](https://github.com/Skanislav/pq-sap/tree/main/lean/docs/): `announcement-model.md` (the
...
unlinkability ≤ sharedSecretHiding true + auxKeyIndependence
  ...
  sprAdv             ≤ epsilonPrim + 3·seeded-MLWE + epsilonEnc + keyRestore
                       per branch; keyRestore ≤ MLWE + keyIdealization
                       (`sprAdv_le_mlwe`)                     [Lean]
spend (related-key) ≤ q·Adv_spendForge(b+η)                   [Lean;
                       offset-subtraction reduction; signature layer not composed]
...
Twenty-four content modules plus the axiom audit and the root.
...
- **`WidenedSigning.lean`** — the widened ML-DSA identification scheme for
  blinded keys: `widenedValidKeyPair` (the pk identity plus each secret a sum
  of two `η`-short vectors; the blinded key qualifies,
  `widenedValidKeyPair_blinded`), the FIPS 204 cube mask sampler
  `sampleMaskCube`, both response gates widened to `β' = 2β`
  (`sampleInBall_smul_widened_bound`), the key identity behind verification
  (`wApprox_eq_of_keyIdentity`), completeness (`widened_ids_complete`), and
  the exact accepted-`z` probability for any `β'`-short shift
  (`cube_shift_accept_prob`, `widened_z_accepted_independent`), instantiated
  as `(1047791/1048576)^1280` at ML-DSA-65. HVZK is stated at the trivial
  `widenedHvzkDistance = 1` — shape fixed, bound not sharpened.
- **`BlindingROM.lean`** — the missing ingredient, modelled: `hashAddr ∘ pack`
  ...
  term is at most the two bad-query probabilities). The `Pr[bad]` query-bound
  half is closed in `BlindingEntropy.lean`; the MLWE hop and the
  `auxKeyIndependence` identification remain the documented gap.
- **`BlindingEntropy.lean`** — the ROM query bound for the address point:
  `blindBadProb ≤ qH·β` under `IsQueryBoundP` (adversary's address-oracle
  queries) and `BlindPointMassBound`, via the cache-key counting of
  `ROMUpToBad` (`roImpl_run_keys_subset`, `probEvent_isSome_apply_le`);
  `blindingAdvantageRO_le_queryBound` (the `2·qH·β` term); and the ML-DSA
  point mass `blindPointMassBound_mldsa` = `(2^d/q)^(k·256)` from the
  concrete `Power2Round` fiber count, instantiated for ML-DSA-65.
...
- **`SPRTwoHop.lean`** — the SPR terms decomposed for ML-KEM-768: three
  seeded decision-MLWE problems (key hop `t = A·s+e` vs uniform; ciphertext
  hop over `[Âᵀ | t̂ᵀ]`; key-restoration hop behind the simulator), genuine
  reduction adversaries, **all hop identities proved**
  (`game0/game1_keyHopProblem`, `game0/game1_ctHopProblem`,
  `game0/game1_keyRestorationAdv`), and `sprAdv_le_mlwe`:
  `sprAdv ≤ epsilonPrim + MLWE + MLWE + epsilonEnc + keyRestore`, with
  `keyRestore ≤ MLWE + keyIdealization`
  (`keyRestoration_le_mlwe_add_keyIdealization`); the residual terms
  (`epsilonPrim`, `epsilonEnc`, `keyIdealization`) are named assumption
  records, unbounded here. Implicit rejection provably never enters.
...
- **`SpendSecurity.lean`** — the related-key spend game for a malicious
  sender who *knows* the `q` blinding offsets it created
  (`relatedSpendRun`/`relatedSpendAdvantage`): the union bound over the
  targeted key (`relatedSpendAdvantage_le_sum`, `…_le_mul`), the
  offset-subtraction reduction (`relatedSpendReduction`,
  `ownershipValid_subtractOffset` — a `b`-short witness for derived key `i`
  minus an `η`-short offset is a `(b+η)`-short master witness), and the ML-DSA
  instance `mldsa_relatedSpendAdvantage_le`. The signature-layer losses
  (`CmaToNmaLossNN`, `TruncationLossNN`, `UnboundedSigningAssumption`) are
  defined with explicit hypotheses, not yet composed.
- **`ConstructionASecurity.lean`** — the assembled capstones:
  `unlinkAdvantage_full_bound` (the scheme-level four-term decomposition with
  every term replaced by its named bound), `unlinkAdvantageMulti_full_bound`
  (`q·εSingle`), and `relatedSpendAdvantage_le_mul_capstone` at ML-DSA.
...
- **`Axioms.lean`** — 156 build-checked `#print axioms` assertions.
...
- Bumping the pin is a reviewed act, not a `lake update`: the checklist, the
  ...
  in [`docs/vcvio-pin.md`](/lean/vcvio-pin).
...
- **Sorry-freedom and the axiom basis** are asserted by
  `PqStealth/Axioms.lean`, which the root module imports. It carries 156
