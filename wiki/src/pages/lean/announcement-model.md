...
:::info[Mirrored page]
Source: [`lean/docs/announcement-model.md`](https://github.com/Skanislav/pq-sap/blob/main/lean/docs/announcement-model.md). Edit the source file; this copy is regenerated by `pnpm sync` in `wiki/`.
:::
...
**Where this stops, precisely.** Two things are open — but only one wholly:
...
* **The identification.** `blindingAdvantageRO` is not connected to
  ...
  about the term itself. This is assumption 9 of
  `docs/SECURITY_ANALYSIS.md` §5.
* `blindBadProb ≤ mlwe + q_H · β` — the query-bound half is now PROVED:
  `BlindingEntropy.blindBadProb_le_queryBound` gives `blindBadProb b ≤ q_H · β`
  for an adversary whose address-oracle queries are bounded (`IsQueryBoundP`),
  via `ROMUpToBad`'s cache-key counting (`roImpl_run_keys_subset`,
  `probEvent_flag_trackingROImpl_pointPolicy_le`) and the union bound
  `probEvent_isSome_apply_le`, under the point-mass hypothesis
  `BlindPointMassBound` — discharged for the concrete `Power2Round` and
  injective `pack` as `blindPointMassBound_mldsa` = `(2^d/q)^(k·256)`. What
  remains is the MLWE hop that makes the mask uniform in the first place,
  reducing `auxKeyIndependence` to the ROM game.
