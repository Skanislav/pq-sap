...
:::info[Mirrored page]
Source: [`docs/TECHNICAL_SPEC.md`](https://github.com/Skanislav/pq-sap/blob/main/docs/TECHNICAL_SPEC.md). Edit the source file; this copy is regenerated by `pnpm sync` in `wiki/`.
:::

Status: pre-freeze working draft, backed by the executable spec in
[`python/`](https://github.com/Skanislav/pq-sap/tree/main/python/) and the conformance vectors in
[`python/vectors/v0/`](https://github.com/Skanislav/pq-sap/tree/main/python/vectors/v0/). Everything stated here is
implemented and tested; open questions are marked as such.

## 1. Scheme overview

Construction A from [plan.md](/overview/plan): ML-KEM (FIPS 203) establishes the
...
[`DECISIONS.md`](/spec/decisions). Headlines:
