...
:::info[Mirrored page]
Source: [`docs/SECURITY_ANALYSIS.md`](https://github.com/Skanislav/pq-sap/blob/main/docs/SECURITY_ANALYSIS.md). Edit the source file; this copy is regenerated by `pnpm sync` in `wiki/`.
:::
...
The ROM abstraction of the blinding term — `blindGameRO` of `BlindingROM.lean`, with the mask already
uniform by construction — is bounded by

```
blindingAdvantageRO ≤ epsilonBlindExpand + Adv_blindMLWE + 2 * qH * betaAddr
```

(this is the `hBlind` hypothesis consumed by `unlinkAdvantage_full_bound`); connecting
`blindGameRO` to `auxKeyIndependence` itself is assumption 9 below.

The proven ROM part: for an adversary making at most `qH` address-oracle queries
(`IsQueryBoundP … qH`) and a point-mass bound `beta ≠ ⊤`,
`PqStealth.ConstructionA.probOutput_blindBadQuery_le` / `blindBadProb_le_queryBound` give
`blindBadProb b ≤ qH * beta` per branch (tracking flag = point cached, at most `qH` cache
keys, union bound over the keys — `ROMUpToBad` § query budget), and
`PqStealth.ConstructionA.blindingAdvantageRO_le_queryBound` yields the `2 * qH * beta` term.
`epsilonBlindExpand` is assumption 1 and `betaAddr` is assumption 2.
...
9. `BlindingIdentification` (not yet a Lean assumption record) — that `blindGameRO` is
   `auxKeyIndependence` after the mask hop: the KEM, the ciphertext and the
   `viewTag`/`expandBlind` derivation are carried through the ROM monad, the mask draw is
   commuted into place, and the mask is made uniform by the seeded-MLWE hop
   (`mlweAdvOfUnlinkAdv` / `mlweAdvOfBlindAdvRO` type-check the two reductions). The
   `qH·β` bound is proven for the uniform-mask ROM game; this identification is what
   connects it to the blinding term of the scheme. Stated as open in
   `BlindingROM.lean` and `lean/docs/announcement-model.md`.
...
These exclusions are also stated in `docs/TECHNICAL_SPEC.md` §9 ("Security analysis
freeze").
